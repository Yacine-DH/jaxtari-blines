...
def evaluate(
    ...
    network = _Network(action_dim=env.action_space().n, n_cos=n_cos)

    key, network_key = jax.random.split(key)
    dummy_obs = env.observation_space().sample(network_key).squeeze()[None, ...]

    dummy_taus = jnp.zeros((dummy_obs.shape[0], n_tau_k))
    q_params = network.init(network_key, dummy_obs, dummy_taus)
    ...
    def get_action(q_params: flax.core.FrozenDict, next_obs: jnp.ndarray, key: jax.random.PRNGKey):
        ...
        quantiles = network.apply(q_params, next_obs, taus)
        q_values = quantiles.mean(axis=1)
